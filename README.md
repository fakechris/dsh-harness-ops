...
**mini TUI 引导模式（`--guide` / 菜单 7，2026-08-13 教训）**：一次无人值守的 `--agent` 长跑
失败过——被误报带偏、超时被杀、什么都没修成。**教训：没有人 guide 的 doctor 长任务不靠谱。**
引导模式是**真正的全屏 TUI**（python3+curses，stdlib，无第三方依赖）：顶栏状态 + 可滚动
主区 + 底部输入条。先跑确定性诊断，逐项修复（`[Y]es/[n]o/[?]/[q]`），然后进入 **LLM 对话**：
输入消息 + 回车 = 跑一轮 headless agent（上下文跨轮携带）；**Ctrl-C 随时打断运行中的 agent**，
输入引导后回车续聊；PgUp/PgDn/Home/End 滚动，Ctrl-L 清屏，`/help` `/quit`。CoT/prompt/终答
一律 **markdown 渲染**（标题/粗体/行内代码/代码块/列表/引用），不是裸文本。默认**不做无人
长跑**；无终端时回退为逐步模式。
...
- **引导模式**（菜单 7）：确定性 + LLM 的**人机协同入口**——修复逐个确认，LLM 只读复核或
  受监督修复；适合不放心无人长跑的场景
